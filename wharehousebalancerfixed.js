...
PATCH (robust Desktop + Mobile):
- Mobile/Desktop detection is now done on the LOADED pages ($page / $prod), not on the current DOM.
- All risky .children[x].innerText / .href access is guarded to prevent “Cannot read properties of undefined”.
- Fixed out-of-range langShinko[16] usage (array ends at index 15).
- Added safe helpers and fallback selectors for village-id extraction.
*/
...
function detectMobileOnPage($p) {
    try {
        if ($p.find("#mobileHeader, #mobile_header").length > 0) return true;
        if ($p.find(".mwood,.mstone,.miron").length > 0) return true;
        // Mobile tables often contain mheader resources markers
        if ($p.find(".mheader.ressources,.icon.mheader").length > 0) return true;
        return false;
    } catch (e) {
        return false;
    }
...
function safeChild(el, idx) {
    if (!el || !el.children || el.children.length <= idx) return null;
    return el.children[idx] || null;
}
...
function safeHref(el) {
    ...
    if (el.getAttribute) return el.getAttribute("href") || "";
    return "";
}
...
.submenu{display:flex;flex-direction:column;position: absolute;left:0px;top:37px;min-width:240px;}
    ...
    // Your original theme-switching block (kept as-is)
    if (colors == 'pink') {
        cssClassesSophie = `
        <style>
        ...
        .btnSophie {linear-gradient(to bottom, #947a62 0%,#7b5c3d 22%,#6c4824 30%,#6c4824 100%);color:white}
        .btnSophie:hover {linear-gradient(to bottom, #b69471 0%,#9f764d 22%,#8f6133 30%,#6c4d2d 100%);color: white}
...
function displayEverything() {
    ...
    $.get(URLIncRes, function () {
        ...
        .done(function (page) {
            ...
            var $page = $(page);
            var incIsMobile = detectMobileOnPage($page);

            //grab all the ressies incoming to each village
            var $trRows = $page.find("#trades_table tr");
            for (var i = 1; i < $trRows.length - 1; i++) {
                var villageData = {};
                var villageIDtemp;

                try {
                    //check whether the HTML layout is mobile or desktop, based on LOADED page
                    if (incIsMobile) {
                        console.log("mobile");

                        // Guarded: row might not have enough children
                        var rowEl = $trRows[i];
                        if (!rowEl) continue;

                        // Skip obvious "System" rows if they exist
                        if (safeText(rowEl).indexOf(langShinko[15]) !== -1) continue;

                        var td5 = safeChild(rowEl, 5);
                        var td5c1 = safeChild(td5, 1);
                        var resourceGroups = td5c1 ? td5c1.children : null;

                        if (resourceGroups) {
                            for (let j = 0; j < resourceGroups.length; j++) {
                                let $child = $($(resourceGroups[j]));
                                let iconClass = $child.find('.icon.mheader').attr('class');
                                if (!iconClass) continue;

                                let classNames = iconClass.split(' ');
                                let resourceType = classNames[classNames.length - 1];
                                let resourceAmount = $child.text().replace(/[^\d]/g, '');

                                if (resourceType === "mwood") villageData.wood = resourceAmount;
                                if (resourceType === "mstone") villageData.stone = resourceAmount;
                                if (resourceType === "miron") villageData.iron = resourceAmount;
                            }
                        }

                        // Village id path (guarded)
                        var td3 = safeChild(rowEl, 3);
                        var td3c2 = safeChild(td3, 2);
                        villageIDtemp = extractVillageIdFromHref(safeHref(td3c2));
                        if (!villageIDtemp) {
                            // fallback
                            var anyLink = $(rowEl).find("a[href*='id=']").get(0);
                            villageIDtemp = extractVillageIdFromHref(safeHref(anyLink));
                        }
                        ...
                        var rowElD = $trRows[i];
                        if (!rowElD) continue;
                        if (safeText(rowElD).indexOf(langShinko[15]) !== -1) continue;

                        var td8 = safeChild(rowElD, 8);
                        var resourceGroupsD = td8 ? td8.children : null;

                        if (resourceGroupsD) {
                            for (let j = 0; j < resourceGroupsD.length; j++) {
                                let $child = $($(resourceGroupsD[j]));
                                var classNames;

                                if ($child[0] && $child[0].innerHTML && $child[0].innerHTML.indexOf("header") > -1) {
                                    var ic = $child.find('.icon.header').attr('class');
                                    if (!ic) continue;
                                    classNames = ic.split(' ');
                                } else {
                                ...
                                if (resourceType === "wood") villageData.wood = resourceAmount;
                                if (resourceType === "stone") villageData.stone = resourceAmount;
                                if (resourceType === "iron") villageData.iron = resourceAmount;
                            }
                        }

                        // Village id (guarded)
                        var td4 = safeChild(rowElD, 4);
                        var td4c0 = safeChild(td4, 0);
                        villageIDtemp = extractVillageIdFromHref(safeHref(td4c0));
                        if (!villageIDtemp) {
                            var anyLinkD = $(rowElD).find("a[href*='id=']").get(0);
                            villageIDtemp = extractVillageIdFromHref(safeHref(anyLinkD));
                        }
                    }
                    ...
                    console.warn("Skipped a trades_table row due to unexpected structure:", i, e);
                    continue;
                }
            }
            ...
            $.get(URLProd, function () {
                ...
                .done(function (page) {
                    ...
                    var $prod = $(page);
                    var prodIsMobile = detectMobileOnPage($prod);

                    uniVillage = $prod.find("span.bonus_icon_33");
                    ...
                    if (prodIsMobile) {
                        console.log("mobile");
                        allWoodObjects = $prod.find(".res.mwood,.warn_90.mwood,.warn.mwood");
                        allClayObjects = $prod.find(".res.mstone,.warn_90.mstone,.warn.mstone");
                        allIronObjects = $prod.find(".res.miron,.warn_90.miron,.warn.miron");
                        allWarehouses = $prod.find(".mheader.ressources");
                        allVillages = $prod.find(".quickedit-vn");
                        allFarms = $prod.find(".header.population");
                        allMerchants = $prod.find('#production_table a[href*="market"]');
                        productionTable = $prod.find("#production_table th");
                        ...
                        for (let i = 0; i < allVillages.length; i++) {
                            // farm
                            var farmTxt = allFarms[i] && allFarms[i].parentElement ? safeText(allFarms[i].parentElement) : "0/0";
                            var fm = farmTxt.match(/(\d*)\/(\d*)/);
                            farmSpaceUsed.push(fm ? fm[1] : "0");
                            farmSpaceTotal.push(fm ? fm[2] : "0");

                            // warehouse: parse digits to be safe
                            var whTxt = allWarehouses[i] && allWarehouses[i].parentElement ? safeText(allWarehouses[i].parentElement) : "0";
                            warehouseCapacity.push(digitsOnlyNumber(whTxt));

                            // merchants: keep legacy; if empty -> 0
                            availableMerchants.push(allMerchants[i] ? safeText(allMerchants[i]) : "0");
                            totalMerchants.push("999");

                            // points
                            var pTxt = productionTable[(i * 2) + 1] ? safeText(productionTable[(i * 2) + 1]) : "0";
                            villagePoints.push(pTxt.replace(/\./g, '').replace(',', ''));
                        }
                    }
                    else {
                        console.log("desktop");
                        allWoodObjects = $prod.find(".res.wood,.warn_90.wood,.warn.wood");
                        allClayObjects = $prod.find(".res.stone,.warn_90.stone,.warn.stone");
                        allIronObjects = $prod.find(".res.iron,.warn_90.iron,.warn.iron");
                        allVillages = $prod.find(".quickedit-vn");
                        if (uniRow >= 0) {
                            allVillages.splice(uniRow, 1);
                            allWoodObjects.splice(uniRow, 1);
                        ...
                        for (let i = 0; i < allVillages.length; i++) {
                            warehouseCapacity.push(digitsOnlyNumber(safeText(allIronObjects[i].parentElement.nextElementSibling)));
                            var mTxt = safeText(allIronObjects[i].parentElement.nextElementSibling.nextElementSibling);
                            var mm = mTxt.match(/(\d*)\/(\d*)/);
                            availableMerchants.push(mm ? mm[1] : "0");
                            totalMerchants.push(mm ? mm[2] : "0");

                            var fTxt = safeText(allIronObjects[i].parentElement.nextElementSibling.nextElementSibling.nextElementSibling);
                            var ff = fTxt.match(/(\d*)\/(\d*)/);
                            farmSpaceUsed.push(ff ? ff[1] : "0");
                            farmSpaceTotal.push(ff ? ff[2] : "0");

                            villagePoints.push(safeText(allWoodObjects[i].parentElement.previousElementSibling).replace(/\./g, '').replace(',', ''));
                        }
                    }

                    //making a useable Data object
                    for (let i = 0; i < allVillages.length; i++) {
                        var vId = allVillages[i] && allVillages[i].dataset ? allVillages[i].dataset.id : null;
                        if (!vId) continue;

                        var vUrl = "";
                        try {
                            vUrl = allVillages[i].children[0].children[0].href;
                        } catch (e) {
                            vUrl = "";
                        }
                        ...
                        villagesData.push({
                            ...
                            "url": vUrl,
                            "name": safeText(allVillages[i]).trim(),
                    ...
                    woodAverage = Math.floor(totalWood / (warehouseCapacity.length || 1));
                    stoneAverage = Math.floor(totalStone / (warehouseCapacity.length || 1));
                    ironAverage = Math.floor(totalIron / (warehouseCapacity.length || 1));
                    ...
                    if (settings.isMinting == false) {
                        ...
                        for (let i = 0; i < warehouseCapacity.length; i++) {
                            actualWoodAverage = Math.floor(actualTotalWood / (actualWHCountNeedsBalancingWood || 1));
                            actualStoneAverage = Math.floor(actualTotalStone / (actualWHCountNeedsBalancingStone || 1));
                            actualIronAverage = Math.floor(actualTotalIron / (actualWHCountNeedsBalancingIron || 1));
                ...
                color: black;"></div>
                    ...
                    //assign excess to shortage (wood)
                    ...
                    //assign excess to shortage (stone)
                    for (let q = shortageResources.length - 1; q >= 0; q--) {
                        $("#progress").css("width", `${(shortageResources.length - q) / shortageResources.length * 100}%`);
                    ...
                    //assign excess to shortage (iron)
                    for (let q = shortageResources.length - 1; q >= 0; q--) {
                        $("#progress").css("width", `${(shortageResources.length - q) / shortageResources.length * 100}%`);
                        for (let d = 0; d < merchantOrders.length; d++) {
                            merchantOrders[d].distance = checkDistance(merchantOrders[d].x, merchantOrders[d].y, villagesData[q].name.match(/(\d+)\|(\d+)/)[1], villagesData[q].name.match(/(\d+)\|(\d+)/)[2]);
...
function showStats() {
    ...
    for (let i = 0; i < stillShortage.length; i++) {
        if (i % 2 == 0) tempRow = " id='" + i + "' class='sophRowB'";
        else tempRow = " id='" + i + "' class='sophRowA'";
...
function resAfterBalance() {
    ...
    for (var i = 0; i < villagesData.length; i++) {
        ...
        if (i % 2 == 0) tempRow = "class='sophRowB'";
        else tempRow = "class='sophRowA'";
